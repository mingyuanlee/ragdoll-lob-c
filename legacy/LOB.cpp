    ...
    if (type == BID) { // buy
        ...
        while (ask_price != 0 && (limit_price != 0 ? ask_price <= limit_price : true) && remaining_vol > 0) {
            ...
            if (remaining_vol >= ask_node->total_volume) {
                ...
                ask_tree.deleteLimitPrice(ask_price);
                remaining_vol -= vol_to_deduct;
                ask_price = (ask_tree.minLimitNode() == nullptr ? 0 : ask_tree.minLimitNode()->limit_price);
            } else {
        ...
        if (remaining_vol > 0) {
            if (limit_price > 0) {
                LimitNode *new_node = bid_tree.insertLimitPrice(limit_price);
                bid_tree.insertOrder(oid, remaining_vol, owner, limit_price);
            } else {
                LimitNode *new_node = bid_tree.insertLimitPrice(last_executed_price);
                bid_tree.insertOrder(oid, remaining_vol, owner, limit_price);
            }
        }
    } else { // sell
